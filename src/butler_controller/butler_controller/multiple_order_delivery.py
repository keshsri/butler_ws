...
class MultipleOrderDeliveryRobot(Node):
    def __init__(self):
        super().__init__('multiple_order_delivery_robot')
        self.state = State.HOME
        self.orders = []
        self.current_order_index = 0
        self.transition_delay = 10.0  # 5 seconds delay between states
        self.order_acceptance_timer = None
        self.order_acceptance_duration = 20.0  # Time window to accept multiple orders
    ...
    def table_delivery_confirmation_callback(self, msg):
        if self.state == State.TO_TABLE and self.current_order_index < len(self.orders):
            ...
            if self.current_order_index < len(self.orders):
                ...
                self.get_logger().info('All deliveries completed. Returning home...')
    ...
    def move_to_table(self):
        if self.current_order_index < len(self.orders):
            ...
            # Simulate delivering to the table (wait for delivery confirmation)
        else:
            self.transition_to(State.RETURN_HOME)
...
def main(args=None):
    ...
    multiple_order_delivery_robot = MultipleOrderDeliveryRobot()
    rclpy.spin(multiple_order_delivery_robot)
    rclpy.shutdown()

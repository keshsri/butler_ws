...
class OrderWithKitchenReturnRobot(Node):
    def __init__(self):
        super().__init__('order_with_kitchen_return_robot')
        self.state = State.HOME
        self.table_number = None
        self.timeout_duration = Duration(seconds=30.0)  # Timeout duration in seconds
    ...
    def delivery_timeout_callback(self):
        self.get_logger().info('Timeout waiting for delivery confirmation. Returning to kitchen...')
        self.cancel_timeout_timer()
    ...
    def return_to_kitchen(self):
        ...
        # Simulate returning to the kitchen
        self.transition_to(State.RETURN_HOME)
        self.return_home()
...
def main(args=None):
    ...
    order_with_kitchen_return_robot = OrderWithKitchenReturnRobot()
    rclpy.spin(order_with_kitchen_return_robot)
    rclpy.shutdown()
